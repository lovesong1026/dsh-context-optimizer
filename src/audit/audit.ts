...
import { hostTranslate, type HostLocaleId, type HostTranslate } from './locale.ts'
...
export function renderReport(report: AuditReport, locale: HostLocaleId = 'en'): string {
  const t: HostTranslate = hostTranslate(locale)
  const sep = t('sep')
  const lines: string[] = []
