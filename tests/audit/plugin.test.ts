...
test('插件入口：apply 注册 context_audit 工具', () => {
  ...
  apply(ctx as never)
  assert.equal(name, 'dsh-context-optimizer')
  assert.deepEqual(inject, ['fs', 'skills', 'tools', 'sessions'])
  assert.equal(registered.length, 6)
  const tool = registered.find((entry) => (entry as { name?: string }).name === 'context_audit') as { name: string; description: string; parameters: { properties: Record<string, unknown> }; execute: Function; output: { render: Function } }
...
test('插件入口：无 webServer 环境（headless）下工具仍注册、路由跳过', () => {
  ...
  apply(ctx as never)
  assert.equal(registered.length, 6)
  const tool = registered.find((entry) => (entry as { name?: string }).name === 'context_audit') as { name: string }
  assert.equal(tool.name, 'context_audit')
})
...
test('插件端到端：execute 产出完整审计报告', async () => {
  ...
  try {
    ...
    ctx.tools.register = (def: unknown) => { registered.push(def) }
    apply(ctx as never)
    const tool = registered.find((entry) => (entry as { name?: string }).name === 'context_audit') as { execute: Function }
    ...
    assert.ok(text.includes('Tool surface optimisation'))
    ...
    assert.ok(schema.description.startsWith('Audit context injected'))
  } finally {
