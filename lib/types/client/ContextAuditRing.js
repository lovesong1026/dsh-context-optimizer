...
import { formatTokens } from "../audit/tokens.js";
...
export function ContextAuditRing(props) {
    ...
    return _jsxs("span", { ref: dockRef, "data-context-optimizer": true, style: dockStyle, children: [_jsx("button", { type: "button", onClick: () => setOpen(value => !value), title: `${t('cd.title')} · ${status}`, "aria-label": `${t('cd.title')} · ${status}`, "aria-expanded": open, "aria-controls": panelId, style: { ...triggerStyle, color: accent }, children: _jsx(PulseIcon, { size: 16 }) }), open && _jsxs("section", { id: panelId, role: "dialog", "aria-label": t('cd.title'), style: panelStyle, children: [_jsxs("header", { style: headStyle, children: [_jsx("span", { style: eyebrowStyle, children: t('cd.title') }), _jsxs("span", { style: { ...statusStyle, color: accent }, children: [_jsx("span", { "aria-hidden": "true", style: { ...statusDotStyle, background: accent } }), status] })] }), state.state === 'error' && _jsxs("p", { style: errorStyle, children: [t('cd.error'), ": ", state.error] }), report === null && state.state !== 'error'
                        ...
                        : report !== null && _jsxs(_Fragment, { children: [_jsxs("div", { style: gaugeStyle, children: [_jsxs("div", { style: readStyle, children: [_jsx("strong", { style: readValueStyle, children: formatTokens(resident) }), _jsx("span", { style: readUnitStyle, children: t('cd.residentUnit') }), _jsxs("span", { style: readPercentStyle, children: [Math.round(percent * 100), "% / ", formatTokens(FULL_SCALE)] })] }), _jsxs("div", { style: railStyle, role: "img", "aria-label": `${formatTokens(resident)} / ${formatTokens(FULL_SCALE)}`, children: [_jsx("span", { style: railTrackStyle, children: segments.filter(segment => segment.tokens > 0).map(segment => _jsx("span", { style: {
                                                            ...
                                                            height: '100%',
                                                        } }, segment.key)) }), THRESHOLDS.map(threshold => _jsx("span", { "aria-hidden": "true", style: { ...tickStyle, left: `${(threshold / FULL_SCALE) * 100}%` }, children: _jsx("span", { style: tickLabelStyle, children: formatTokens(threshold) }) }, threshold))] })] }), optimization !== undefined && _jsxs("section", { style: optimizationStyle, "aria-label": t('cd.optimization'), children: [_jsxs("header", { style: optimizationHeadStyle, children: [_jsx("span", { style: optimizationTitleStyle, children: t('cd.optimization') }), _jsx("span", { style: { ...tierStyle, color: optimization.active ? TONE.mint : TONE.muted }, children: optimization.active ? t('cd.tier', { tier: optimization.tier }) : t('cd.inactive') })] }), _jsxs("div", { style: optimizationStatsStyle, children: [_jsxs("span", { style: optimizationStatStyle, children: [_jsx("span", { style: optimizationLabelStyle, children: t('cd.before') }), _jsx("strong", { style: optimizationValueStyle, children: formatTokens(optimization.registeredSchemaTokens) }), _jsxs("small", { style: optimizationSubStyle, children: [optimization.registeredTools, " tools"] })] }), _jsx("span", { style: optimizationArrowStyle, "aria-hidden": "true", children: "\u2192" }), _jsxs("span", { style: optimizationStatStyle, children: [_jsx("span", { style: optimizationLabelStyle, children: t('cd.after') }), _jsx("strong", { style: optimizationValueStyle, children: formatTokens(optimization.visibleSchemaTokens + optimization.manifestTokens) }), _jsxs("small", { style: optimizationSubStyle, children: [optimization.visibleTools, " schemas + ", optimization.deferredTools, " deferred"] })] }), _jsxs("span", { style: savingStyle, children: [_jsx("span", { style: optimizationLabelStyle, children: t('cd.savings') }), _jsx("strong", { style: { ...optimizationValueStyle, color: optimization.savedTokens > 0 ? TONE.mint : TONE.muted }, children: formatTokens(optimization.savedTokens) }), _jsx("small", { style: optimizationSubStyle, children: optimization.registeredSchemaTokens === 0 ? '0%' : `${Math.round((optimization.savedTokens / optimization.registeredSchemaTokens) * 100)}%` })] })] }), _jsx("div", { style: savingRailStyle, "aria-label": `${t('cd.before')} ${formatTokens(optimization.registeredSchemaTokens)}, ${t('cd.after')} ${formatTokens(optimization.visibleSchemaTokens + optimization.manifestTokens)}`, children: _jsx("span", { style: { ...savingRailVisibleStyle, width: `${Math.min(100, optimization.registeredSchemaTokens === 0 ? 0 : ((optimization.visibleSchemaTokens + optimization.manifestTokens) / optimization.registeredSchemaTokens) * 100)}%` } }) }), _jsxs("div", { style: warmStyle, children: [_jsx("span", { style: warmLabelStyle, children: t('cd.warm') }), _jsx("span", { style: warmValueStyle, title: optimization.warmTools.join(', '), children: optimization.warmTools.length === 0 ? t('cd.none') : optimization.warmTools.join(', ') })] })] }), _jsx("ul", { style: tableStyle, children: segments.map(segment => {
...
const triggerStyle = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 30, height: 30, padding: 0, background: 'transparent', border: `1px solid ${TONE.border}`, borderRadius: 7, cursor: 'pointer', font: 'inherit' };
...
const optimizationStyle = { margin: '15px 16px 0', padding: '11px 12px 10px', background: TONE.raised, border: `1px solid ${TONE.border}`, borderRadius: 8 };
const optimizationHeadStyle = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 };
const optimizationTitleStyle = { color: TONE.text, fontSize: 11.5, fontWeight: 600 };
const tierStyle = { fontFamily: MONO, fontSize: 10.5, fontVariantNumeric: 'tabular-nums' };
const optimizationStatsStyle = { display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 14px minmax(0, 1fr) minmax(0, .85fr)', alignItems: 'end', gap: 7, marginTop: 10 };
const optimizationStatStyle = { display: 'flex', minWidth: 0, flexDirection: 'column', gap: 2 };
const optimizationLabelStyle = { color: TONE.quiet, fontSize: 9.5, fontWeight: 500 };
const optimizationValueStyle = { color: TONE.text, fontFamily: MONO, fontSize: 14, fontWeight: 600, fontVariantNumeric: 'tabular-nums' };
const optimizationSubStyle = { overflow: 'hidden', color: TONE.muted, fontSize: 9.5, textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
const optimizationArrowStyle = { paddingBottom: 7, color: TONE.quiet, fontSize: 12, textAlign: 'center' };
const savingStyle = { display: 'flex', minWidth: 0, flexDirection: 'column', gap: 2, paddingLeft: 8, borderLeft: `1px solid ${TONE.border}` };
const savingRailStyle = { height: 5, marginTop: 10, overflow: 'hidden', background: TONE.sunk, borderRadius: 99 };
const savingRailVisibleStyle = { display: 'block', height: '100%', background: TONE.amber, borderRadius: 99, transition: 'width 180ms ease' };
const warmStyle = { display: 'grid', gridTemplateColumns: '72px minmax(0, 1fr)', gap: 8, marginTop: 9, paddingTop: 8, borderTop: `1px solid ${TONE.border}` };
const warmLabelStyle = { color: TONE.quiet, fontSize: 10.5 };
const warmValueStyle = { overflow: 'hidden', color: TONE.muted, fontFamily: MONO, fontSize: 10.5, textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
