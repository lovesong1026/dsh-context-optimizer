 ...
 */
export function parseRuntimeFile(text, path) {
    let raw;
    try {
        raw = JSON.parse(text);
    }
    ...
    if (record.version !== undefined) {
        if (typeof record.version !== 'number')
            throw new Error(`dsh-context-optimizer: ${path}: "version" must be a number`);
    ...
    if (record.groups !== undefined) {
        if (!Array.isArray(record.groups))
            throw new Error(`dsh-context-optimizer: ${path}: "groups" must be an array`);
        const groups = record.groups.map((entry) => {
            ...
            if (typeof group.name !== 'string')
                throw new Error(`dsh-context-optimizer: ${path}: each group needs a string "name"`);
        ...
        if (reason !== undefined)
            throw new Error(`dsh-context-optimizer: ${path}: invalid groups: ${reason}`);
