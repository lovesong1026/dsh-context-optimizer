import { buildGroupSummary, buildGroupedManifest } from "./groups.js";
...
export function computeTier(input) {
    if (input.deferredSize === 0)
        return 0;
    if (!input.forced && input.catalogSize <= input.minCatalogSize)
        return 0;
    if (input.manifestTokens <= input.budget)
        return 1;
    if (input.namesOnlyTokens <= input.budget)
        return 2;
    return 3;
}
/** The listing budget: `min(thresholdPct% × contextWindow, listingMaxTokens)`. */
export function budgetFor(thresholdPct, contextWindow, listingMaxTokens) {
 ...
 */
export function computeDisclosure(entries, eagerNames, bridgeNames, tier) {
    if (tier === 0) {
 ...
 */
export function applyDisclosure(assembly, disclosure) {
    if (disclosure.tier === 0)
        return assembly;
