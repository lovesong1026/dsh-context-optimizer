 ...
 */
export async function rerank(matcher, query, documents, timeoutMs) {
 ...
 */
export async function searchCatalog(matcher, query, entries, limit, timeoutMs) {
    if (entries.length === 0)
        return [];
 ...
 */
export function keywordSearch(query, entries, limit) {
    if (entries.length === 0)
        return [];
