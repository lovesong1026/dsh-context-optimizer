...
export const FORCED_EAGER = ['tool_search', 'tool_describe', 'tool_call', 'tool_slimmer_catalog', 'tool_slimmer_update_config', 'skill'];
/**
 ...
 */
export function groupFor(toolName, groups) {
    for (const group of groups) {
        if (group.tools?.includes(toolName))
            return group.name;
        if (group.prefixes?.some(prefix => toolName.startsWith(prefix)))
            return group.name;
    }
    return undefined;
}
 ...
 */
export function buildGroupedManifest(entries, mode) {
 ...
 */
export function buildGroupSummary(entries) {
