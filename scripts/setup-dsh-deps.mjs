...
/** Executable links need to live in this package's `.bin` lookup path. */
const BIN_LINKS = {
  'node_modules/.bin/vitest': 'node_modules/.bin/vitest',
}
...
function relink(target, source, type = 'dir') {
  let current
  try {
...
for (const [target, rel] of Object.entries(BIN_LINKS)) {
  const source = join(checkout, rel)
  if (existsSync(source)) relink(join(ROOT, target), source, 'file')
}
